...
export default function AdminLayout({ eyebrow, title, description, actions, children }) {
  ...
  const links = [
    ...
    ...(user?.role === "owner" || user?.is_break_glass ? [{ to: "/admin/recycle-bin", label: "Recycle Bin" }] : []),
  ];
    ...
    <div className="admin-shell">
      <header className="admin-header">
        <NavLink to="/admin" className="admin-brand" aria-label="Climoraone admin dashboard">
          ...
          <span className="admin-brand-copy"><small>Private workspace</small><strong>Commerce Studio</strong></span>
        </NavLink>
        <nav className="admin-nav" aria-label="Admin navigation">
          {links.map((link) => <NavLink key={link.to} to={link.to} end={link.end} className={({ isActive }) => (isActive ? "active" : "")}>{link.label}</NavLink>)}
          ...
          <div className="admin-user-menu" ref={menuRef}>
            <button type="button" className="admin-user-trigger" onClick={() => setMenuOpen((open) => !open)} aria-expanded={menuOpen}>
              ...
              <span className="admin-user-copy"><strong>{user?.name || "Administrator"}</strong><small>{roleLabel(user?.role)}</small></span><span aria-hidden="true">⌄</span>
            </button>
            {menuOpen && <div className="admin-user-dropdown">
      ...
      <main className="admin-main">
        <section className="admin-hero"><div><span className="admin-eyebrow">{eyebrow || "Climoraone administration"}</span><h1>{title}</h1>{description && <p>{description}</p>}</div>{actions && <div className="admin-page-actions">{actions}</div>}</section>
        {children}
      </main>
    </div>
  );
