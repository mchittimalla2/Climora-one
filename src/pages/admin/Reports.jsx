...
function Reports() {
  ...
  const loadReports = async () => {
    try {
      ...
      setError(reportError.message || "Unable to load report data.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <AdminLayout
      ...
      description="A live operational view of sales, inventory health, order movement and product performance."
      actions={<button className="admin-secondary-btn" onClick={loadReports}>Refresh data</button>}
    >
      {error && <div className="admin-alert error">{error}</div>}
      {loading ? (
        <div className="admin-panel admin-empty">Preparing live business insights...</div>
      ) : (
        <>
          ...
          <section className="admin-grid-2">
            <article className="admin-panel">
              <div className="admin-panel-header"><div><span className="admin-eyebrow">Product demand</span><h2>Best-selling products</h2></div></div>
              {analytics.topProducts.length === 0 ? (
                <div className="admin-empty">Sales rankings will appear after orders are placed.</div>
              ) : (
                analytics.topProducts.map((product) => (
                  <div className="admin-progress-row" key={product.name}>
                    <div className="admin-progress-label"><strong>{product.name}</strong><span>{product.quantity} sold · {money(product.revenue)}</span></div>
                    <div className="admin-progress-track"><div className="admin-progress-fill" style={{ width: `${(product.quantity / maxTopQuantity) * 100}%` }} /></div>
                  </div>
                ))
              )}
            </article>
            ...
            <article className="admin-panel">
              <div className="admin-panel-header"><div><span className="admin-eyebrow">Inventory watch</span><h2>Products needing attention</h2></div></div>
              {[...analytics.outOfStock, ...analytics.lowStock].length === 0 ? (
                <div className="admin-empty">Inventory is healthy. No low-stock alerts.</div>
              ) : (
                <div className="admin-table-wrap">
                  <table className="admin-table">
                    <thead><tr><th>Product</th><th>Category</th><th>Stock</th><th>Status</th></tr></thead>
                    <tbody>
                      {[...analytics.outOfStock, ...analytics.lowStock].slice(0, 8).map((product) => (
                        <tr key={product.id}>
                          <td><strong>{product.name}</strong></td>
                          <td>{product.category || "Collection"}</td>
                          <td>{Number(product.stock || 0)}</td>
                          <td>{Number(product.stock || 0) <= 0 ? "Out of stock" : "Low stock"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
