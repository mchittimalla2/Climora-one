      ...
      <table className="product-table">
        ...
        <tbody>
          {products.length === 0 ? (
            ...
            const image = product.images?.[0] || product.main_image || `${import.meta.env.BASE_URL}images/logo.jpeg`;
            const details = parseProductDetails(product.description || "");
            return (
              <tr key={product.id}>
                <td><img src={image} alt={product.name} className="product-thumb" loading="lazy" />{product.images?.length > 1 && <small className="product-image-count">{product.images.length} images</small>}</td>
                <td><strong>{product.name}</strong><p>{details.summary}</p></td>
